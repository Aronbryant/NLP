...
class FeedbackModel(tez.Model):
    ...
    def forward(self, ids, mask, token_type_ids=None, targets=None):
        ...
        sequence_output = transformer_out.last_hidden_state  # 取最后一层所有token的embedding
...
if __name__ == "__main__":
    ...
    training_samples = prepare_training_data(train_df, tokenizer, args, num_jobs=NUM_JOBS)  # ids,label
    valid_samples = prepare_training_data(valid_df, tokenizer, args, num_jobs=NUM_JOBS)

    train_dataset = FeedbackDataset(training_samples, args.max_len, tokenizer)  # ids,mask,targets
